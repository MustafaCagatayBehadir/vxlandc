import ncs
import l1access.helpers as helpers
from .resource_manager import id_allocator


def create_port_parameters(root, port, tctx, port_parameters, log):
    ...
    port_parameters['description'] = port.description if port.description else helpers.create_description(
        port, port_parameters)
    port_parameters['bum'] = helpers.get_bum(port_parameters)
    ...
    if port_parameters['type'] == 'ethernet':
        ...
        port_parameters['vpc-node'] = 'TRUE' if helpers.is_node_vpc(
            port, port_parameters) else 'FALSE'
    elif port_parameters['type'] == 'port-channel':
        port_parameters['port-channel-id'] = id_allocator.id_read(
            tctx.username, root, helpers.get_port_channel_id_pool_name(root, port), f'{port.site}::{port.tenant}::{port.name}')
        ...
        port_parameters['port-channel-id'] = id_allocator.id_read(
            tctx.username, root, helpers.get_port_channel_id_pool_name(root, port), f'{port.site}::{port.tenant}::{port.name}')
        port_parameters['node-1'], port_parameters['node-2'] = helpers.get_vpc_nodes(
            port)
    ...
    # Set hidden elements
    set_hidden_leaves(port, port_parameters)

    # Create Node Port Flat Lists for Query
    set_node_port_flat_leaf_list(port, port_parameters)


def set_node_port_flat_leaf_list(port, port_parameters):
...
def set_hidden_leaves(port, port_parameters):
...
def allocate_po_id(root, port, tctx, log):
    """Function to allocate port-channel id from resource manager
...
def create_port(port, port_parameters):
    ...
    if port_parameters['type'] == 'ethernet':
        ...
        for node_port in port_parameters['node-port']:
            ...
            helpers.apply_template(port, 'ethernet', tvars)
    elif port_parameters['type'] == 'port-channel':
        tvars.add('DEVICE', port_parameters['node'])
        tvars.add('PO_ID', port_parameters['port-channel-id'])
        tvars.add('VPC', port_parameters['is-vpc'])
        helpers.apply_template(port, 'port-channel', tvars)
        for node_port in port_parameters['node-port']:
            tvars.add('ETH_ID', node_port)
            tvars.add('PO_MEMBER_DESCRIPTION', helpers.create_po_member_description(
                node_port, port_parameters))
            helpers.apply_template(port, 'port-channel-member', tvars)
    elif port_parameters['type'] == 'vpc-port-channel':
        tvars.add('PO_ID', port_parameters['port-channel-id'])
        tvars.add('VPC', port_parameters['is-vpc'])
